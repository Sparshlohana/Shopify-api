...
// 1. code: A reference to the shipping method
// 2. discounted_price: The price of the after line level discounts have been applied. Doesn't reflect cart and order level discounts
// 3. discounted_price_set: The price of the shipping method in both shop and presentment currencies after line level discounts have been applied.
// 4. price: The price of the shipping method in the shop currency. Can't be negative.
// 5. price_set: The price of the shipping method in shop and presentment currency.
// 6. source: The source of the shipping method.
// 7. title: The title of the shipping method.
// 8. tax_lines: A list of tax lines objects, each of which details a tax applicable to this shipping line.
// 9. carrier_identifier: A reference to the carrier service that provided the rate. Present when the rate was computed by the third party carrier service.
// 10. request_fulfillment_service_id: A reference to fulfillment service that is being requested for the shipping method. Present if the method requires 3rd party fulfillment services or else it is null.

// 55. source_name
// The source of the checkout, To use this field for the sales attribution, you must register the channels that your app is managing.You can register the channels that your app is managing by completing google forms.After you have submitted the request, you need to wait for the request to be processed by the shopify.You can find a list of your channels in the partner's dashboard, in your apps marketplace extension. You can specify a handle as the source_name value in your request.

// 56. source_identifier
// The id of the order placed on the originating platform. This value does not correspond to that is generated by after you complete a draft.

// 57. source_url
// A valid URL to the original order on the originating surface. The URL is displayed to merchants on the order detail page. If the URL is invalid, then it won't be displayed.

// 58. subtotal_price
// The price of the order in the shop currency after discounts but before shipping, duties, taxes, and tips.

// 59. subtotal_price_set
// The subtotal of the order in the shop and presentment currencies after discounts but before shipping, duties, taxes, and tips.

// 60. tags
// Tags attached to the order, formatted as string od comma separated values. Tags are additionally short of descriptors, commonly, used for filtering and searching purposes. Each individual tag is limited to 40 characters in length.

// 61. tax_lines
// An array of tax line objects, each of which details a tax applicable order. Each object have the following properties:
// 1. price: The amount of tax charged in the shop currency.
// 2. rate: The rate of tax to be applied.
// 3. title: The name of the tax.
// 4. channel_liable: Whether the channel that submitted the tax lines is liable for remitting. A value of null indicates unknown liability for the tax line.

// 62. tax_included
// Whether the taxes are included in the order subtotal.

// 63. test
// Whether this is an test order.

// 64. token
// A unique value when referencing to the order.

// 65. total_discount
// The total discounts applied to the price of order in the shop currency.

// 66. total_discount_set
// The total discounts applied to the price of order in the shop and presentment currency.

// 67. total_line_items_price
// The sum of all line items price in the shop currency.

// 68. total_line_items_price_set
// The total of all line items prices in the shop and presentment currencies.

// 69. total_outstanding
// The total outstanding amount in the shop currency.

// 70. total_price
// The sum of all line items prices, discounts, shipping, taxes, and tips in the shop currency. It must be positive.

// 71. total_price_set
// The total price of the order in shop and presentment currencies.

// 72. total_shipping_price_set
// The total shipping price of an order, excluding discounts and returns, in shop and presentment currencies. If tax_included is set to be true, then total_shipping_price_set includes taxes.

// 73. total_tax
// The sum of all the taxes applied to the order in the shop currency. It must be positive.

// 74. total_tax_set
// The total tax applied to the order and presentment currency.

// 75. total_tax_received
// The sum of all the tips in the order in the shop currency.

// 76. total_weight
// The sum of all line weights in grams. The sum is not adjusted as items are removed from the order.

// 77. updated_at
// The date and time when the order was last updated. You should not filter the orders using updated_at.

// 78. user_id
// The ID of the user logged into shopify POS (point of sale) who processed the order if applicable.
